...
- (NSDictionary*)emptyRecord
{
    ...
    // create dummy activity records
    //NSMutableDictionary* tempActivityRecords = [[NSMutableDictionary alloc] initWithObjectsAndKeys:@"sweetrolls stolen", @"none yet", nil];
    //[tempRecord setValue:tempActivityRecords forKey:EKRecordActivityRecordsKey];
...
- (NSMutableDictionary*)activityRecords
{
    ...
    return [record objectForKey:EKRecordActivityRecordsKey];
}
...
- (void)setActivityRecords:(NSMutableDictionary*)input
{
    ...
    if( input != nil ) {
        [record setValue:input forKey:EKRecordActivityRecordsKey];
    }
}*/
...
{
    NSDictionary* rootDictionary = EKDictionaryFromFile(filename);
    if( rootDictionary == nil || rootDictionary.count < 1 ) {
        NSLog(@"[EKRecord] WARNING: Could not load flags; it wasn't possible to load the root dictionary from the file named [%@]", filename);
        return;
    }
    
    if( shouldOverride == YES ) {
        NSLog(@"[EKRecord] DIAGNOSTIC: Will forcibly overwrite existing flags with flags from file: %@", filename);
    } else {
        NSLog(@"[EKRecord] DIAGNOSTIC: Will add flags (without overwriting) from file named: %@", filename);
    }
    
    for( NSString* key in rootDictionary.allKeys ) {
        id value = [rootDictionary objectForKey:key];
        
        if( shouldOverride == YES ) {
            // just write the value in no matter what
            [self setFlagValue:value forFlagNamed:key];
        } else {
            id existingFlag = [self flagNamed:key];
            // in "no overwrite" mode, only set the value if no existing flag is found
            if( existingFlag == nil ) {
                [self setFlagValue:value forFlagNamed:key];
            }
        }
